...
###initialize with gaus values ##need to research approx astronomical distributions, suns, planets, etc.
 ...
 #   G = 6.674 * 10 ** -11
    ...
    for i in range(iterations):
        ...
        velocities  =   np.random.normal(1,1,[n_bodies,dim])             #, dtype=np.float64)  # m/s x,y for each of n bodies
        mass_array  =   10**15 * np.random.normal(10,1,[n_bodies])        #, dtype=np.float32)  # masses for each of n bodies, in kg

        pos_array.astype(np.float64)
        velocities.astype(np.float64)
        print(type(pos_array[1,1]))
...
init_gaus(604800, 2, 3, time_slice=3600)
